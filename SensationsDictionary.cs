...
namespace OWO_H3VR
{
    public static class SensationsDictionary
    {
        ...
        public static string SensationByWeaponName(string weaponName)
        {
            string prefix = "Pistol";            

            if (rocketRounds.Any(weaponName.Contains)) { prefix = "Rocket"; }
            if (shotgunRounds.Any(weaponName.Contains)) { prefix = "Shotgun"; }
            if (rifleRounds.Any(weaponName.Contains)) { prefix = "BigRifle"; }
            if (battleRifleRounds.Any(weaponName.Contains)) { prefix = "Rifle"; }
            if (assaultRifleRounds.Any(weaponName.Contains)) { prefix = "Rifle"; }
